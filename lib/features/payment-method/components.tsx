...
const Actions = ({ data }: { data: UnitType }) => {
	...
	const [deleting, { isLoading }] = useDeletePaymentMethodMutation();
	...
	return (
		...
		>
			<DropDownDotItem
				...
				name="Edit"
				onChange={() => {
					router.push(`/settings/financial/payment-method/edit-${data._id}`);
				}}
				disabled={loading}
			/>
			<DropDownDotItem
				...
				name="View"
				onChange={() => {
					router.push(`/settings/financial/payment-method/${data._id}`);
				}}
				disabled={loading}
			/>
			<DropdownMenuSeparator />
...
const Add = () => {
	return (
		<TableItem.AddLink
			href="/settings/financial/payment-method/create"
			icon="PlusCircle"
			text="Add Method"
		/>
	);
