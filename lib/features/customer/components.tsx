...
import { confirm, status } from '@/lib/actions';
import { showToast, ToastOptions } from '@/lib/actions/tost';
import { ArrowUpDown } from 'lucide-react';
import { Badge } from '@/components/ui/badge';

const Column: ColumnDef<CustomerType>[] = [
	TableItem.SelectBox(),
	TableItem.ImageIcon(),
...
const Actions = ({ data }: { data: CustomerType }) => {
	const router = useRouter();
	const params = useParams<{ slug: string; item: string }>();

	const [deleting, { isLoading }] = useDeleteCustomerMutation();
	...
	return (
		...
		>
			<DropDownDotItem
				...
				name="Edit"
				onChange={() => {
					router.push(`/peoples/customers/edit-${data._id}`);
				}}
				disabled={loading}
			/>
			<DropDownDotItem
				...
				name="View"
				onChange={() => {
					router.push(`/peoples/customers/${data._id}`);
				}}
				disabled={loading}
			/>
			<DropdownMenuSeparator />
